...
class StatusCheckAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        logger.debug("[StatusCheckAgentExecutor] execute exiting")
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        ...
        if session_id in self._active_sessions:
            logger.info(
                f"Cancellation requested for active status check session: {session_id}"
            )
            # TODO: Implement proper cancellation when ADK supports it
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f"Cancellation requested for inactive status check session: {session_id}"
            )
